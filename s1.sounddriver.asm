...
; Sound_Play:
CycleSoundQueue:
		movea.l	Go_SoundPriorities(pc),a0
		lea	v_soundqueue0(a6),a1	; load music track number
		move.b	v_sndprio(a6),d3	; Get priority of currently playing SFX
		moveq	#3,d4			; Number of queues-1 (v_soundqueue0, v_soundqueue1, v_soundqueue2, v_soundqueue3)
; loc_71F12:
@inputloop:
...
SegaPCM_Pitch:  equ $04

; Sound_E1: PlaySega:
PlaySegaSound:
		move.b  #$B6,d0    ; Register: FM3/6 Panning
                move.b  #$C0,d1    ; Value: Enable both channels
                bsr.w   WriteFMII   ; Write to YM2612 Port 1 (for FM6) [sub_72764]
		...
		move.l	#(SegaPCM_End-SegaPCM)-1,d3			; Load the size of the SEGA PCM sample into d3
                lea     (ym2612_a0).l,a3
		move.b	#$2A,(a3)		; Write to DAC channel
PlayPCM_Loop:
		move.b	(a2)+,1(a3)		; Write the PCM data (contained in a2) to $A04001 (YM2612 register D0)
		moveq	#SegaPCM_Pitch,d0	; Write the pitch to d0
		...
		btst	#7,(v_jpadhold1).w	; Check for Start button input
...
@bgm_loadMusic:
		...
		movea.l	Go_SpeedUpIndex(pc),a4
		subq.b	#bgm__First,d7
		move.b	(a4,d7.w),v_speeduptempo(a6)
		movea.l	Go_MusicIndex(pc),a4
		lsl.w	#2,d7
...
Sound_PlaySpecial:
		...
		tst.b	f_fadein_flag(a6)	; Is music being faded in?
		bne.w	@locret			; Exit if it is
		movea.l	Go_SpecSoundIndex(pc),a0
...
PSGDoVolFX_Loop:
		...
		bmi.s	VolEnvHold			; If over $80, branch
; loc_72960:
@gotflutter:
